...
| Положение RTG/RMG                    | регистр `ПоложениеRTG_RMG`, регламент `ПолучитьПоложениеКрановRTG_RMG` → `ОбновлениеОнлайнДанныхТерминала.ОбновитьПоложениеКрановRTG_RMG`; команда viewer `updateYardCranePositions` |
...
| RTG/RMG  | Краны площадки без мониторинга Скаут. Положение — по ячейке контейнера из УКТ (`uktId` в `yard-cranes-v4.json`) | Регламент `ПолучитьПоложениеКрановRTG_RMG` раз в 20 с пишет `ПоложениеRTG_RMG`. Форма тем же polling шлёт `updateYardCranePositions`. Без строки УКТ кран остаётся над последней ячейкой |
...
- Сцена V4 в реальном режиме собирается из макетов плюс совместимых снимков EDS/УПП/УКТ. Форма раз в 20 с одним серверным вызовом читает снимки, регистр Скаут и `ПоложениеRTG_RMG`; команды viewer `updateVessels` / `updateRailWagons` / `updatePlacements` / `updateScoutVehicles` / `updateYardCranePositions`. Запись снимков EDS/УПП/УКТ — отдельные регламенты; запись Скаут — `ПолучитьТехникуСкаут`; запись кранов — `ПолучитьПоложениеКрановRTG_RMG`.
