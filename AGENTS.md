...
| Источник | Что даёт |
|---|---|
| EDS | суда у причалов (`ЕДС_ЖурналШвартовыхОпераций`) и рейд (`ЕДС_Рейс`) |
| УКТ | наполнение площадок (`ОстаткиКонтейнеров`): агрегаты `Зона` / `Стек` / `Ряд` / `Ярусов`; ярус рисуется как условные 20 ft |
| ВМКТ | доп. сведения по контейнеру в карточке ячейки |
| УПП | вагоны (`ОстаткиВагоновНаЖДПутях`) и техника |
| Скаут | GPS в `ПоложениеТехникиСкаут` (регламент `ПолучитьТехникуСкаут` раз в 20 с, хранение 1 ч); форма V4 читает срез последних тем же интервалом. При выключенном флаге — демотехника из `ObjectModule` |
