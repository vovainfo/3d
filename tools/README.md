...
Проверяет источники сцены V4, включая здания, QGIS-слои виртуальных причалов и портальных кранов, ручные реестры `data/vessels-v4.json` и `data/yard-cranes-v4.json`, заполняет расчётные значения `sites[].expected`, приводит полигоны площадок к вычисленным размерам и синхронизирует результат с текстовыми макетами обработки `d3_v4`.
...
Режим `--check` не изменяет файлы и показывает планируемые корректировки. Обычный запуск дополнительно формирует `data/berths-v4.json` и единый `data/cranes-v4.json`, обновляет соответствующие макеты. Пример ручного реестра RTG/RMG находится в `docs/yard-cranes-v4.example.json`. Скрипт не изменяет BSL-модули и не затрагивает V3. При ошибках исходники и макеты не обновляются; манифест записывается последним и связывает комплект макетов общим `buildId`.
