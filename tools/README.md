...
Проверяет источники сцены V4, заполняет расчётные значения `sites[].expected`, приводит полигоны площадок к вычисленным размерам и синхронизирует результат с текстовыми макетами обработки `d3_v4`.
...
Режим `--check` не изменяет файлы и показывает планируемые корректировки. Обычный запуск записывает рассчитанные значения в `data/terminal-layout-v4.json`, а увеличенную геометрию — в слои площадок и направляющих. Скрипт не изменяет BSL-модули и не затрагивает V3. При ошибках исходники и макеты не обновляются; манифест записывается последним и связывает комплект макетов общим `buildId`.
