...
Проверяет источники сцены V4, включая здания, QGIS-слои виртуальных причалов и ручной реестр `data/vessels-v4.json`, заполняет расчётные значения `sites[].expected`, приводит полигоны площадок к вычисленным размерам и синхронизирует результат с текстовыми макетами обработки `d3_v4`.
...
Режим `--check` не изменяет файлы и показывает планируемые корректировки. Обычный запуск дополнительно объединяет `berth_points_v4.geojson` и `berth_heading_axes_v4.geojson` в `data/berths-v4.json`, обновляет макеты `BerthsV4_json` и `VesselsV4_json`. Скрипт не изменяет BSL-модули и не затрагивает V3. При ошибках исходники и макеты не обновляются; манифест записывается последним и связывает комплект макетов общим `buildId`.
